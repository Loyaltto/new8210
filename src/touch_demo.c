...
void touch_demo(void){
	...
	pszDeviceName = getenv("TSLIB_TSDEVICE");
	if (NULL == pszDeviceName)
		pszDeviceName = "/dev/input/by-path/platform-touchscreen-event";
	lcd_flip();
	dev = ts_open(pszDeviceName, 1);
	if (NULL != dev){
		ts_config(dev);
				 	...
				 	if (kb_hit()){
						switch(kb_getkey()){
							...
							case 0x0D:
                                ...
                                if(ifd < 0)//sy 20140902 19:12
                                {
                                    lcd_clean();
                                    lcd_printf(ALG_LEFT, "Device Open failed");
                                    lcd_printf(ALG_LEFT, "Module Not Support");
                                    lcd_flip();
                                    kb_getkey();
                                    return;
                                }
